...
def get_train_val_test_loader(dataset, collate_fn=default_collate,
                              batch_size=256, train_val_test_ratio=(0.6, 0.2, 0.2), num_workers=10,
                              pin_memory=True):

    total_size = dataset.size

    print("total_size in train_and_val_loader: ", total_size)
    ...
    print(f"train_size: {train_size}/{total_size} = {train_ratio}")
    print(f"val_size: {val_size}/{total_size} = {val_ratio}")
    print(f"test_size: {test_size}/{total_size} = {test_ratio}")
...
def multi_graph_collate_fn(batch):
    # Unzip the batch: each element is ([graph1, ..., graph9], target, idx)
    list_of_graph_lists, targets, idx_list = zip(*batch)

    # Transpose the list of lists so that we group graphs by position
    # i.e., all first-model graphs together, all second-model graphs together, etc.
    # length 9, each is a list of graphs
    graph_groups = list(zip(*list_of_graph_lists))

    # Convert each group into a PyG Batch
    batched_graphs = [Batch.from_data_list(graphs) for graphs in graph_groups]

    # Stack the targets and indices
    batch_targets = torch.stack(targets)  # already torch.Tensor from get()
    idx_tensor = torch.tensor(idx_list)

    return batched_graphs, batch_targets, idx_tensor
