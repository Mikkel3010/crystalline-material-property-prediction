...
class ConcatModel(nn.Module):
    ...
    def forward(self, data_list):
        outputs = [model(data) for model, data in zip(
            self.base_models, data_list)]
        concat = torch.cat(outputs, dim=1)

        prediction = self.meta_model(concat)
...
class Finetune(object):
    def __init__(self, config):
        ...
        base_dir = 'rerun_Ensemble_finetune_concat'
...
if __name__ == "__main__":
    ...
    if config["do_training"] and config["do_testing"]:
        print("make sure that you want train and test the model, preduction using model after training")
        print(f" {config['do_training']} training")
        print(f" {config['do_testing']} testing")
