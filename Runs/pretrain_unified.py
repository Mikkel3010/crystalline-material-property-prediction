...
class HDFCrystalGraphDataset(Dataset):
    def __init__(self, config):
        super().__init__()
        self.config = config
        with h5py.File(list(config["dataset_paths"].values())[0], 'r') as f:
        ...
        self.dataset_paths = [
            (representation, path,) for representation, path in config['dataset_paths'].items() if path.endswith('.h5')]
        print("dataset_paths", self.dataset_paths)
        print("amount of representations", len(self.dataset_paths))
        self.representation_count = len(self.dataset_paths)
    ...
    def get(self, idx):
        ...
        dataset_index = np.random.choice(self.representation_count)
        representation = self.dataset_paths[dataset_index][0]
        dataset_path = self.dataset_paths[dataset_index][1]
...
class Pretrain_run(object):
    def __init__(self, config):
        ...
        log_dir = os.path.join('shuffle_pretrain_runs',
                               config["task_name"], dir_name)
...
if __name__ == "__main__":
    config_path = "Data_process/config_files/shuffle_runs/pretrain/all_shuffle_method.yaml"
