...
class MultiheadModel(nn.Module):
    ...
    def forward(self, data_list):
        ...
        outputs = [m(x) for m, x in zip(self.base_models, data_list)]

        outputs = torch.stack(outputs, dim=1)

        attn_output, _ = self.self_attn(outputs, outputs, outputs)

        scores = self.fuse_attn(attn_output)

        attn_weights = F.softmax(scores, dim=1)

        fused_embed = torch.sum(attn_weights * attn_output, dim=1)

        pred = self.meta_model(fused_embed)
...
class Finetune(object):
    def __init__(self, config):
        ...
        base_dir = 'rerun_pytorch_attention_Ensemble_finetune'
